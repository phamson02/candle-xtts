...
impl SelfAttention {
    fn new(cfg: &GPT2LayerConfig, vb: VarBuilder) -> Result<Self> {
        let c_attn = linear(cfg.n_embd, cfg.n_embd * 3, vb.pp("c_attn"))?;
...
impl MLP {
    fn new(cfg: &GPT2LayerConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let c_fc = linear(cfg.n_embd, hidden_dim, vb.pp("c_fc"))?;
        let c_proj = linear(hidden_dim, cfg.n_embd, vb.pp("c_proj"))?;
        Ok(Self { c_fc, c_proj })
    }
}
...
impl GPT {
    ...
    fn prepare_attention_mask_for_generation(
        &self,
        inputs: &Tensor,
        pad_token_id: Option<&Tensor>,
        eos_token_id: Option<&Tensor>,
    ) -> Result<Tensor> {
        let default_attention_mask = Tensor::ones(
            (inputs.shape().dim(0)?, inputs.shape().dim(1)?),
            candle_core::DType::I64,
            inputs.device(),
        )?;

        if pad_token_id.is_none() {
            return Ok(default_attention_mask);
        }
